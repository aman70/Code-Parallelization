 ...
 * Host code.
 * 
 * Author: Naga Kandasamy and Aryaman Sinha
 * Date: 02/14/2017
 */
...
MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    printf("Setting up a %d x %d grid of thread blocks. \n", (Pd.width + 1 - TILE_SIZE*TILE_SIZE/(TILE_SIZE*TILE_SIZE)),(Pd.height/(TILE_SIZE*TILE_SIZE)));

    	dim3 grid((Pd.width + 1 - TILE_SIZE*TILE_SIZE/(TILE_SIZE*TILE_SIZE)),(Pd.height/(TILE_SIZE*TILE_SIZE)));

//	dim3 grid((Pd.width + TILE_SIZE - 1)/TILE_SIZE, (Pd.height + (TILE_SIZE)^2 - 1)/(TILE_SIZE)^2);
